...
angular.module('app').controller('SituationController', function ($scope, AdminService, ServiceService, chosenCategory, messageBusService, statesRepository, $rootScope, $sce, $anchorScroll, TitleChangeService, $location) {
  ...
  $scope.trustAsHtml = function (string) {
    console.log("$scope.trustAsHtml = function (string) {");

    if(statesRepository.isKyivCity()){
      return $sce.trustAsHtml(string.replace(new RegExp('igov.org.ua', 'g'), 'central.es.kievcity.gov.ua').
        replace(new RegExp('iGov.org.ua', 'g'), 'central.es.kievcity.gov.ua').
        replace(new RegExp('iGOV.org.ua', 'g'), 'central.es.kievcity.gov.ua').
        replace(/\b[Ii][Gg][Oo][Vv]\b/g, 'KievCity').
        replace(new RegExp('central\.es\.kievcity\.gov\.ua\/wf\/', 'g'), 'igov.org.ua/wf/'));
    } else {
