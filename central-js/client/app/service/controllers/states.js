angular.module('app')
    .controller('ServiceFormController', function ($scope, service, regions, AdminService,
        ...
        var data = CatalogService.getServiceTags(sServiceName).then(function (res) {
            // console.log(res);
            if (Array.isArray(res) && res.length !== 0 && !$rootScope.isOldStyleView) {
...
angular.module('app').controller('SituationController', function ($scope, AdminService, ServiceService, chosenCategory, messageBusService, statesRepository, $rootScope, $sce, $anchorScroll, TitleChangeService, $location) {
    ...
    $scope.category.aServiceTag_Child = $scope.category.aServiceTag_Child.map(function (val) {
        var to = val.sName_UA.indexOf(']');
        val.sName_UA = val.sName_UA.substr(to + 1);
        return val
    });

    try {
        $scope.situationHeader = $scope.category.aServiceTag_Child[0].sNote.split('<h1 class="topic">')[1];
        $scope.situationHeader = $scope.situationHeader.split("</h1>")[0];
        $scope.category.aServiceTag_Child[0].sNote = $scope.category.aServiceTag_Child[0].sNote.replace('<h1 class="topic">'+$scope.situationHeader+'</h1>', "")
    }catch (e){console.log(e);}
    $scope.printParams = window.state.params;
...
angular.module('app').controller('TitleChange', function ($scope, TitleChangeService) {
    $scope.$on('$stateChangeSuccess', function (event, toState) {
        if (!$state.is('index.situation') || !$state.is('index.print') || !$state.is('index.newsubcategory') || !$state.is('index.service') || !$state.is('index.oldbusiness') || !$state.is('index.subcategory')) {
