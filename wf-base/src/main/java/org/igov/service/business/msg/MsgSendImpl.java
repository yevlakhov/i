...
public class MsgSendImpl implements IMsgSend {
	...
	public MsgSendImpl(String sURL, String sBusId, String sType, String sFunction, String sTemplateId, String sLogin ) {
		...
		sURL_MSG = sURL;
		sBusinessId_MSG = sBusId;
		sCode_MSG = oMsgType.getAbbr() + "-" + sf;
		this.sFunction = sFunction;
		sTemplateId_MSG = sTemplateId;
		sLogin_MSG = sLogin;

		LOG.debug("MsgCode={}", sCode_MSG);
	}
	...
	public <T> IMsgSend _Params(Map<String, T > mParam) {//HashMap<String, T > mParam
		if ( mParam != null ) {
			...
			mParam.entrySet().stream().forEach((o) -> {
				asParam.add(o.getKey() + ": " + (o.getValue()==null?"NULL":o.getValue())+"");
			});
		}
	...
	public IMsgSend addLangFilter(String lang) {
		try {
			oMsgLang = MsgLang.valueOf(lang.trim().toUpperCase());
		} catch (final IllegalArgumentException e) {
			oMsgLang = MsgLang.UKR;
		}
		LOG.debug("Received lang={}, set lang={}", lang, oMsgLang);
		return this;
	}
	...
	private String buildJSON() {
		...
		sb.append(sTemplateId_MSG);
		sb.append("\", \"ext\":{\"LocalMsg\":[{\"Level\":\"");
		sb.append(oMsgLevel);
		sb.append("\",\"Lang\":\"");
		sb.append(oMsgLang.name());
		sb.append("\",\"Text\":\"");
		sb.append(sFunction);
		sb.append("\",\"FullText\":\"\"}]}}}]}");
	...
	public IMsgObjR save() throws Exception {
		...
		if ( retMsg != null ) {
			...
			if (retMsg.getMsgCode().equals(MSG_DEFAULT) && !sCode_MSG.equals(MSG_DEFAULT)) {
				LOG.warn("Сообщение с кодом {} не найдено, попытка его создания.", sCode_MSG);
				createMsg();
				LOG.info("Созданно сообщение с кодом : {}", sCode_MSG);
